...
def run_nationality_prompt_on_chunk(person_name: str, chunk_text: str, cfg_path: Path) -> str:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run Cohere nationalityfinder prompt manually.")
    parser.add_argument("--person", required=True)
    parser.add_argument("--chunk", required=True)
    parser.add_argument("--config", default="config_01.json")
    args = parser.parse_args()

    out = run_nationality_prompt_on_chunk(args.person, args.chunk, Path(args.config))
    print(out)